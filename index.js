...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.biy4zxs.mongodb.net/?retryWrites=true&w=majority`;
...
async function run(){
     try{
          ...
          // getting bookings value according to email 
          ...
          app.post('/doctors',verifyJWT,verifyAdmin, async(req, res)=>{
               ...
               const result = await doctorsCollections.insertOne(query);
               res.send(result);
          })
...
app.listen(port, ()=>{
     console.log(`docotos server is running at ${port}`)
})